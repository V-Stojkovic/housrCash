...
import { addCredit, addPayment, getCashBackRate, getId, getPayments } from '../../../../DBHelpers/db_helpers';
    ...
    try {
        ...
        if (knexInstance && typeof knexInstance.transaction === 'function') {
            const data = await knexInstance.transaction(async (trx: any) => {
                ...
                const payment = await addPayment({ userId: user_id, reference, paymentAmount: amount }, trx);
            ...
            const payment = await addPayment({ userId: user_id, reference, paymentAmount: amount });
