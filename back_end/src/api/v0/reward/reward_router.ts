...
router.get('/', async (req, res) => {
  try {
    ...
    sql = 'SELECT * FROM reward WHERE id > ? AND is_active = 1 ORDER BY id DESC LIMIT ?';
    params = [cursor, limit];
    } else {
    sql = 'SELECT * FROM reward WHERE is_active = 1 ORDER BY id DESC LIMIT ?';
    params = [limit];
    }
    ...
    console.log('Fetched rewards, nextCursor:', nextCursor);
